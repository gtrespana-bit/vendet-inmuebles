...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Propiedades en alquiler en ${estadoName} - VendeT Inmuebles`,
    description: `Encuentra propiedades en alquiler en ${estadoName}. Casas, apartamentos, locales y más.`
  }
}

export default async function AlquilerPorEstadoPage({ params, searchParams }: PageProps) {
  const { locale, estado } = await params
  ...
  const breadcrumbItems = [
    ...
    { label: t('breadcrumbs.for_rent'), href: `/${locale}/propiedades/alquiler` },
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold mb-6">
        Propiedades en alquiler en {stateData.name}
      </h1>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {ciudades.map((ciudad: any) => (
              <Link
                key={ciudad.id}
                href={`/${locale}/propiedades/alquiler/${estado}/${ciudad.id}`}
