...
export default function DashboardPage() {
  ...
  async function handleBoost(productId: string) {
    ...
    if (error || !result?.ok) {
      ...
      const { data: prods } = await supabase.from('vw_propiedades_publicas').select('id, titulo, price, estado, categoria, city, state, activo, visitas, creado_en, main_image_url, operation_type, destacado, destacado_hasta, boosteado_en, estado_moderacion').eq('user_id', user!.id).order('creado_en', { ascending: false })
