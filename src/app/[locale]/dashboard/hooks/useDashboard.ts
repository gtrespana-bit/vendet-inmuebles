...
export function useDashboard() {
  ...
  const refreshAll = useCallback(() => {
    ...
    Promise.all([
      ...
      supabase.from('favoritos').select('producto_id, creado_en, vw_propiedades_publicas!inner(id, titulo, price, main_image_url, activo, user_id, city, state)').eq('user_id', user.id).order('creado_en', { ascending: false }).then(({ data }: { data: any[] | null }) => {
        setFavoritos(data || [])
